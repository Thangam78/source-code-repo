tasks:
  - type: bash_task
    task_id: download_git_files
    bash_command: |
      #!/bin/bash
      set -e
      
      # Input parameters - Set these as environment variables or pass via Airflow params
      GIT_PAT="${GIT_PAT}"
      REPO_URL="${REPO_URL}"
      BRANCH_NAME="${BRANCH_NAME}"
      FILES_PATH="${FILES_PATH}"
      DOWNLOAD_FOLDER="${DOWNLOAD_FOLDER}"
      IS_BASE64_ENCODED="${IS_BASE64_ENCODED:-false}"
      ENCODE_FILES_BASE64="${ENCODE_FILES_BASE64:-false}"
    ...
    env:
      ...
      REPO_URL: "{{ var.value.repo_url }}"
      BRANCH_NAME: "{{ var.value.branch_name }}"
      FILES_PATH: "{{ var.value.files_path }}"
      DOWNLOAD_FOLDER: "{{ var.value.download_folder }}"
      IS_BASE64_ENCODED: "{{ var.value.is_base64_encoded }}"
      ENCODE_FILES_BASE64: "{{ var.value.encode_files_base64 }}"
